...
export class VirtualTourLib {
  ...
  moveMark(panoId) {
    ...
    this.eventFunction({ type: VirtualTourLib.EVENTS.NAV_TO, data: this.activeIndex });
  }
          ...
          texture: loader.load(
            ...
            (t) => {
              ...
              if (!i) {
                ...
                this.eventFunction({ type: VirtualTourLib.EVENTS.INIT, data: t });
              }
            }
          ),
          index: pano.index
  ...
  changeMeshRotation(y) {
    this.meshModel.rotation.y = y;
    // this.transitionMesh.rotation.y = y;
    this.eventFunction({ type: VirtualTourLib.EVENTS.ROTATION_CHANGE, data: this.meshModel.rotation.y });
  }
   ...
   * @param config
   */
  createScene(canvas: HTMLCanvasElement, config: any, callback): void {
    this.eventFunction = callback;
    ...
    this.OrbitControls.addEventListener('end', (e) => {
      this.eventFunction({ type: VirtualTourLib.EVENTS.CHANGE, data: e.target });
    ...
    this.camera.addEventListener('zoom', (e) => {
      this.eventFunction({ type: VirtualTourLib.EVENTS.ZOOM, data: e.target.object.fov });
    })
  }
